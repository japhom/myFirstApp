...
class Boards extends React.PureComponent {
    ...
    fnDeleteBoard = (board) => {
        ...
        fetch('http://www.programacionapi.com:8000/api/deleteBoard/' + idBoard, { method: 'POST', mode: 'cors' })
            .then((response) => {
                return response.json();
            }).then((save) => {
                if (save.status !== "ok") {
    ...
    addBoard = () => {
        ...
        let idBoard = 0;
        fetch('http://www.programacionapi.com:8000/api/addBoard/' + this.state.newBoard, { method: 'POST', mode: 'cors' })
            .then((response) => {
                return response.json();
            }).then((save) => {
                if (save.status !== "ok") {
                    ...
                    console.log(save);// esperando para validar si se guarda o no en BD
                    const nextState = produce(this.state, (draft) => {
                        let newBoard = {
                            items: [],
                            input: "",
    ...
    onAddButtonClick = (object) => {
        ...
        const nextState = produce(this.state, (draft) => {
            ...
            fetch('http://www.programacionapi.com:8000/api/saveItem/' + draft.boards[object].idBoard + "/" + draft.boards[object].input, { method: "POST", mode: "cors" })
                .then((response) => {
                    return response.json();
                }).then((save) => {
                    if (save.status !== "ok") {
